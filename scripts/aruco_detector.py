...
class camFeed():
    def __init__(self):
            ...
            self.s=String()
            self.sub2 = rospy.Subscriber("/iris/usb_cam/camera_info", CameraInfo, self.globaler)
            self.sub1 = rospy.Subscriber("/iris/usb_cam/image_raw", Image, self.callback)
            self.mat = None
            self.dist = None
    ...
    def callback(self,data):
        ...
        if len(marker_corners) > 0 :
            print("Got Aruco")
            rVec, tVec = self.my_estimatePoseSingleMarkers(marker_corners, MARKER_SIZE)
